...
double CalcoloPhi(int I[], int Q[], int n, int fs) {
    // Vettore ampiezza
    double A[n]; 
...
double CalcoloTOF(int fs, double varphi) {

    double dt;
    double tof;

    dt = varphi - prev_varphi;
    if (dt > PI) {
        dt = -1 * copysign(2 * PI - fabs(dt), dt);
    }
        if (dt >= 130.0 / 180.0 * PI || fabs(dt) >= 120 / 180 * PI) {
        dt = prev_dt;
    }

    tof = prev_tof + dt * (1 / (2 * PI * fs)) * 1000000;
    
    printf("Time of flight: %.8f - %.8f - %.8f\n",prev_tof, dt, tof);
    printf("Fase: %.8f\n", prev_varphi);
    printf("Prev_varphi:%.8f, varphi:%.8f\n", prev_varphi,varphi);
...
int main() {
    ...
    int first_iteration_0 = 0;
    int first_iteration_1 = 0;
    while (1) {
        ...
        if (bytesRead > 0) {
            if (receivedChar == '\n') {
                    ...
                    if (Misure_0 >0 && Misure_1>0){ //significa che ho fatto già la doppia misura
                        ...
                        if (calibrazione==1){ 
                            ...
                            if(cont_cal==10){
                                ...
                                // tempi di volo e fase mediati fra le 10 misurazioni prese dalla calibrazione da inserire come primo step nella fase di misurazione 
                ...
                if(sampleIndex==Nsamples && Sensore==Porta ){
                    ...
                    phi_corrente = CalcoloPhi(I, Q, n, Frequenza);
                    ...
                    if(Sensore==0){
                        ...
                        if(calibrazione==1){
                            ...
                            // Detecting first iteration:
                            // During first iteration we use calibration values
                            if (first_iteration_0 == 0)
                            {
                                first_iteration_0   = 1;
                                prev_tof            = tof_cal0;
                                prev_varphi         = phi_cal0;
                                prev_dt             = 0;
                            }
                            
                            CalcoloTOF(Frequenza, phi_corrente);
                            tof_corrente = prev_tof;
                            // phi_corrente = prev_varphi;
                        ...
                        if(calibrazione==1){
                            ...
                            // Detecting first iteration:
                            // During first iteration we use calibration values
                            if (first_iteration_1==0)
                            {
                                first_iteration_1 = 1;
                                prev_tof          = tof_cal1;
                                prev_varphi       = phi_cal1;
                                prev_dt           = 0;
                            }
